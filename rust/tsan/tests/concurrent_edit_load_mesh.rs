//! ThreadSanitizer target for the `SharedVoxelData` worker bridge.
//!
//! Mirrors `voxel-core/tests/threaded_edit_load_mesh_stress.rs` but lives in
//! the `tsan` crate so it can be built with `-Zsanitizer=thread` without
//! pulling in `criterion` (whose `zerocopy` proc-macro dependency is
//! incompatible with the sanitizer runtime). Exercises the same path the
//! production terrain/mesh workers use: concurrent mesh-block tasks on a
//! shared `SharedVoxelData` plus two mutator threads performing region-locked
//! edits and block inserts.
//!
//! Run with:
//! ```text
//! RUSTFLAGS="-Zsanitizer=thread -Cunsafe-allow-abi-mismatch=sanitizer" \
//!   cargo +nightly test -p tsan --test concurrent_edit_load_mesh -- --test-threads=1
//! ```
...
impl VoxelMesher for StressMesher {
    fn build(&self, _output: &mut MesherOutput, input: &MesherInput<'_>) {
        ...
        // A short sleep widens the window in which mesh tasks overlap on the
        // shared data, giving TSan more opportunity to observe cross-thread
...
#[test]
fn concurrent_edit_load_mesh_keeps_data_and_region_locks_consistent() {
