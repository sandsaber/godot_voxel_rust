...
mod tests {
    ...
    /// Writes a distinct SDF value per call (`-(call_index + 1)`), so tests
    /// can detect value mixing across neighbour regions. On the first call it
    /// also writes a marker into the Type channel and leaves Type untouched
    /// afterwards: the gather scratch must be reset between regions, or the
    /// marker leaks into every later region's Type data — the documented B4
    /// regression (leftover data leaking into channels the generator leaves
    /// at their default).
    ...
    impl VoxelGenerator for CountingDistinctSdfGenerator {
        fn generate_block(&self, input: VoxelQueryData<'_>) -> GenResult {
            ...
            input
                .buffer
                .clear_channel_f(ChannelId::Sdf.index(), -((n as f32) + 1.0));
            if n == 0 {
                input.buffer.clear_channel(ChannelId::Type.index(), 7);
            }
            GenResult::default()
        }
    }
    ...
    fn generate_missing_regions_fills_every_missing_neighbour_with_one_scratch() {
        ...
        try_gather_voxels_cpu(
            ...
            1,
            1,
            (1u32 << ChannelId::Sdf.index()) | (1u32 << ChannelId::Type.index()),
            Some(&generator),
            &data,
            0,
            Vector3i::zero(),
        ...
        assert!(
            total_calls >= 26,
            "expected at least 26 generator calls for missing neighbours, got {total_calls}"
        );
        // The scratch buffer is reused across regions (B4) and must be reset
        // between them. Two independent guards:
        // 1. Each region is generated by one call writing a single distinct
        //    negative SDF, so every neighbour region of `dst` must hold
        //    exactly one distinct negative SDF, and no two regions may share
        //    it (value mixing across regions).
        // 2. The generator marks the Type channel only on its first call: a
        //    missing per-region reset would leak that marker into every later
        //    region's Type data, so exactly one region may contain it.
        let dst_size = dst.size();
        let type_default = dst.channel_default(ChannelId::Type.index());
        let mut region_values: Vec<f32> = Vec::new();
        let mut marker_regions = 0usize;
        for dz in -1..=1 {
            for dx in -1..=1 {
                for dy in -1..=1 {
                    if dx == 0 && dy == 0 && dz == 0 {
                    ...
                    let region_box = Box3i::new(
                        Vector3i::new(dx, dy, dz) * bs + Vector3i::splat(1),
                        Vector3i::splat(bs),
                    )
                    .clipped(Box3i::new(Vector3i::zero(), dst_size));
                    if region_box.is_empty() {
                        continue;
                    }
                    let mut distinct: Vec<f32> = Vec::new();
                    let mut has_marker = false;
                    for p in region_box.iter_cells_zxy() {
                        let v = dst.get_voxel_f(p.x, p.y, p.z, ChannelId::Sdf.index());
                        if v < 0.0 && !distinct.contains(&v) {
                            distinct.push(v);
                        }
                        if dst.get_voxel(p.x, p.y, p.z, ChannelId::Type.index()) != type_default {
                            has_marker = true;
                        }
                    }
                    assert_eq!(
                        distinct.len(),
                        1,
                        "neighbour region at ({dx},{dy},{dz}) holds {distinct:?} negative SDF \
                         values; more than one means generated values mixed across regions"
                    );
                    let value = distinct[0];
                    assert!(
                        !region_values.contains(&value),
                        "negative SDF {value} appears in more than one neighbour region"
                    );
        ...
        assert_eq!(
            region_values.len(),
            total_calls,
            "every generator call's value must land in exactly one distinct neighbour region"
        );
        assert_eq!(
            marker_regions, 1,
            "the Type marker written by the first generator call must survive in exactly one \
             neighbour region; more means stale scratch data leaked across regions, zero means \
             the marker was lost"
        );
    }
